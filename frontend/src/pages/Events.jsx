...
export default function EventsPage() {
  ...
  useEffect(() => { setPage(1); }, [severity, domain]);
    ...
    <div className="px-8 py-7 max-w-7xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{total.toLocaleString()} total events</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-white/[0.03] border-b border-white/[0.07]">
              <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium uppercase tracking-wider">Severity</th>
              <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium uppercase tracking-wider">Score</th>
              <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium uppercase tracking-wider">Domain</th>
              ...
              <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium uppercase tracking-wider">Patterns</th>
              <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium uppercase tracking-wider">Preview</th>
              <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium uppercase tracking-wider">Time</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/[0.04]">
            ...
            {!loading && events.length === 0 && (
              <tr><td colSpan={8} className="px-5 py-8 text-center text-gray-500">No events found</td></tr>
            )}
            {!loading && events.map(ev => (
              <tr key={ev._id} className="hover:bg-white/[0.02] transition-colors group">
                <td className="px-5 py-3"><SeverityBadge severity={ev.severity} /></td>
                <td className="px-5 py-3"><ScorePill score={ev.riskScore} /></td>
                <td className="px-5 py-3 text-gray-300 font-mono text-xs">{ev.domain}</td>
                <td className="px-5 py-3">
                  <span className="text-xs text-gray-400 bg-white/[0.05] px-2 py-0.5 rounded">{ev.eventType}</span>
                </td>
                <td className="px-5 py-3 text-gray-400 text-xs">
                  {ev.userRef?.name ?? ev.userId?.slice(0, 8) ?? '—'}
                  {ev.userRef?.department && (
                    <span className="text-gray-600 ml-1">· {ev.userRef.department}</span>
                  )}
                </td>
                <td className="px-5 py-3">
                  {ev.findingLabels?.slice(0, 2).map(l => (
                    <span key={l} className="text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 rounded px-1.5 py-0.5 mr-1">
                      {l}
                    </span>
                  ))}
                  {(ev.findingLabels?.length ?? 0) > 2 && (
                    <span className="text-[10px] text-gray-500">+{ev.findingLabels.length - 2}</span>
                  )}
                </td>
                <td className="px-5 py-3 max-w-[200px]">
                  {ev.maskedPreview && (
                    <code className="text-[10px] text-gray-500 truncate block font-mono">{ev.maskedPreview}</code>
                  )}
                </td>
